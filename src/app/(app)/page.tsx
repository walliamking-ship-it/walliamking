          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
            {loading ? (
              ...
              <div className="p-8 text-center text-gray-400 text-sm">
                暂无销售订单 · <Link href="/sales-orders" className="text-blue-600 hover:underline">去新建</Link>
              </div>
            ) : (
                  ...
                  {recentSales.map((order, i) => (
                    <tr key={order.id} className={`border-b border-gray-50 hover:bg-blue-50/30 ${i !== recentSales.length - 1 ? 'border-b' : ''}`}>
                      <td className="px-4 py-2.5">
                        <Link href="/sales-orders" className="text-blue-600 font-mono text-xs hover:underline">{order.单号}</Link>
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                          order.收款状态 === '全部收款' ? 'bg-green-50 text-green-700' :
                          order.收款状态 === '部分收款' ? 'bg-orange-50 text-orange-700' :
                      ...
                      <td className="px-4 py-2.5 text-center">
                        <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${
                          order.送货状态 === '全部送货' ? 'bg-green-50 text-green-700' :
                          order.送货状态 === '部分送货' ? 'bg-orange-50 text-orange-700' :
                          'bg-red-50 text-red-700'
                        }`}>
                          {order.送货状态 || '未送货'}
                        </span>
