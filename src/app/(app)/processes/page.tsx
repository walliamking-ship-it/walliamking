...
const columns: Column<Process>[] = [
  ...
  { key: 'unitPrice', label: '单价（元）', sortable: true },
...
function ProcessForm({ value, onChange }: { value: Partial<Process>; onChange: (key: keyof Process, v: any) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      <div>
        <label className="block text-xs text-gray-500 mb-0.5">单价（元）</label>
        <input type="number" step="0.01" value={value.unitPrice ?? ''} onChange={e => onChange('unitPrice', parseFloat(e.target.value) || 0)}
...
function FormModal({ open, onClose, onSave, initial }: { open: boolean; onClose: () => void; onSave: (item: Partial<Process>) => void; initial?: Partial<Process>; }) {
  const [form, setForm] = useState<Partial<Process>>(initial || {});
  useEffect(() => { setForm(initial || {}); }, [initial, open]);
  if (!open) return null;
  const handleSave = () => { if (!form.name) { alert('请填写工艺名称'); return; } onSave(form); onClose(); };
  return (
...
export default function ProcessesPage() {
  ...
  const tableColumns: Column<Process>[] = [...columns, { key: 'actions', label: '操作', render: (item) => (
